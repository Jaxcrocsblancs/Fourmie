...
public class Worker
{
	...
	//actualisation gradient
	this.actualisationGradient();
	
	if (etat == Etat.RECHERCHE) {
		//nourriture à notre emplacement
		if(terrain.isFoodAtPoint(currentPos)){
			//recupération et passage en etat retour
			terrain.updateFood(currentPos);
			etat = Etat.RETOUR;
			//Retour vers la colonie
			newDirection = this.retourGradient();
		}
		else{
			//recherche nourriture à proximité
			newDirection = searchForFood();
			//si food pas trouver déplacement aléatoire
			if(newDirection == null){
				//suis les phéromones jusqu'a la ressources
		...
		//test si on est sur la colonie
		if(terrain.isColonyAtPoint(currentPos)){
			//dépot de la ressources
			dropFood();
			etat = Etat.RECHERCHE;
			//Retour vers la ressources
			newDirection = searchForFoodPheromone(null);
		}
		else{//si on est pas sur la colonie
			//on dépose une trace et se dirige vers elle
			dropFoodPheromone();
			newDirection = this.retourGradient();
		}
    ...
    public void actualisationGradient(){
    	int gradient = 1000000000;
    	int x=0,y=0;
    	
    	//recuperation du meilleur gradient autour de soi
    	for(int i =-1; i<2;i++){
    		for(int j =-1; j<2;j++){
        		if(Math.abs(i)+Math.abs(j)==1){
        			if(terrain.getGradient(new Position(currentPos.x+i, currentPos.y+j))<gradient){
        	    		gradient = terrain.getGradient(new Position(currentPos.x+i, currentPos.y+j));
        	    		x= i;
        	    		y= j;
        			}
        		}
    	...
    	//application à toutes les cases
    	for(int i =-1; i<2;i++){
    		for(int j =-1; j<2;j++){
        		if(Math.abs(i)+Math.abs(j)!=2){
        			terrain.setGradient(new Position(currentPos.x+i, currentPos.y+j), gradient+Math.abs(x-i)+Math.abs(y-j));
        		}
        	}
    	}
    ...
    public Direction retourGradient(){
    	int gradient = 1000000000;
    	Direction newDirection = null;
    	Position p = null;
    	//recuperation du meilleur gradient autour de soi
    	for(int i =-1; i<2;i++){
    		for(int j =-1; j<2;j++){
    			if(Math.abs(i)+Math.abs(j)==1)
        			if(terrain.getGradient(new Position(currentPos.x+i, currentPos.y+j))<gradient){
        	    		gradient = terrain.getGradient(new Position(currentPos.x+i, currentPos.y+j));
        	    		p = new Position(currentPos.x+i, currentPos.y+j);
        			}
        	}
    	}
        	
    	
    	newDirection = direction(p);
    	return newDirection;
    }
}
