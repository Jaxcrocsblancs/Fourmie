...
public class Terrain extends JPanel
{
	...
	if(!initFromFile("")){ //map_maze2.map")){
		...
		gradient = new int[panelWidth][panelHeight];
		initGradient();
    }
    ...
    private void initGradient(){
    	...
    	gradient[base.getX()][base.getY()] =0;
    }
    

    public void setGradient(Position pos, int gradient){
    	if(pos.x>0 && pos.x<panelWidth && pos.y>0 && pos.y<panelHeight){
    		if(!isObstacleAtPoint(pos)){
		    	if(this.gradient[pos.x][pos.y]>gradient){
		    		this.gradient[pos.x][pos.y]=gradient;
		    	}
    		}
    	}
    }
    
    public int getGradient(Position pos){
    	if(pos.x>0 && pos.x<panelWidth && pos.y>0 && pos.y<panelHeight)
    		return this.gradient[pos.x][pos.y];
		return 100000000;
    }
    
    public void visuGradient(){
    	for(int i=0;i<panelWidth;i++){
    	    for(int j=0;j<panelHeight;j++) {
    	    	int nb = gradient[i][j];
    	    	if(nb<10)
